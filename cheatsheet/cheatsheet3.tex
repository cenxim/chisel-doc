...
\verb$x <> y$ & Bulk connect \verb$x$ and \verb$y$, control wires \\
 & directionalityis automatically inferred \\
\end{tabular}
...
& & parsed \verb$String$, or declare unassigned if missing \\
...
\subsection{Counters } count up to a specified constant
\begin{scala}
  val counter = Counter(42)
  io.out := someVec(counter.value)
  when(counter.inc()) {
    // counter has reached max, do something
  }
}
\end{scala}
...
\subsection{Instances}Anytime something with a chisel type is
instantiated, some block of hardware will be created.  In order
for that hardware block to become part of the overall generated
circuit it must be connected directly or indirectly to a Module.
...
Each time such a function is called more circuitry will be generated.
...
\begin{scala}
sum1 := Adder(UInt(1), some_data)
sum2 := Adder(UInt(3), some_other_data)
\end{scala}
This creates two adder circuits, one for each time Adder is called.
...
\begin{scala}
  val size = 100
  val u = Reg(Vec(size, UInt(width = 32)))
  val v = Reg(Vec(size, UInt(width = 32)))
  for(i <- 0 to size) {
    v(i) := v(i) * u(i)
  }
\end{scala}
The for statment above will generate 100 multiplication circuits updating the registers specified
%\columnbreak
